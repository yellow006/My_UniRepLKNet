...
class DilatedReparamBlock(nn.Module):
    ...
    def __init__(self, channels, kernel_size, deploy, use_sync_bn=False, attempt_use_lk_impl=True):
        ...
        self.lk_origin = get_conv2d(channels, channels, kernel_size, stride=1,
                                    ...
                                    attempt_use_lk_impl=attempt_use_lk_impl)   # vanilla conv2d implementation
        ...
        if not deploy: # deploy: default = false
            self.origin_bn = get_bn(channels, use_sync_bn)  # get_bn = vanilla batchnorm2d
            for k, r in zip(self.kernel_sizes, self.dilates):
                self.__setattr__('dil_conv_k{}_{}'.format(k, r),  # dynamically set attribute for object itself (DilRepBlock).
    ...
    def forward(self, x):
        if not hasattr(self, 'origin_bn'):      # deploy mode. if deploy == false, we have origin_bn attribute.
...
class UniRepLKNetBlock(nn.Module):
    ...
    # 重参数化：等价于删除了 Dialted Reparam Blocks 中的并行分支、Batch Norm 层和 GRN 中的偏差项。仅在训练完成后使用，用于推理。
    # 理论上不影响训练性能和推理性能，但应该可以加快一点推理速度。
